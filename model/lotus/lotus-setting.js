...
class LotusSetting {
    ...
    if (type === 'def') {
      ...
      if (!fs.existsSync(configFile) && fs.existsSync(defFile)) {
        try {
          ...
          logger.info(`[Lotus插件] 已创建配置文件: ${name}.yaml`)
        } catch (error) {
          logger.error(`[Lotus插件] 创建配置文件失败: ${error.message}`)
        }
      }
      return configFile
    }
  ...
  getYaml(name, type = 'config') {
    ...
    if (!fs.existsSync(filePath)) {
      logger.warn(`[Lotus插件] 配置文件不存在: ${filePath}`)
      return {}
    }
    ...
    try {
      ...
      logger.error(`[Lotus插件] 读取配置文件失败 [${name}]: ${error.message}`)
      return {}
    }
  }
    ...
    try {
      ...
      if (fs.existsSync(filePath)) {
        ...
        try {
          ...
          // 如果解析失败，使用常规方式
          logger.warn(`[Lotus插件] 无法保留注释，使用常规保存: ${parseError.message}`)
      ...
      logger.info(`[Lotus插件] 保存配置成功: ${name}.yaml`)
      return true
    } catch (error) {
      logger.error(`[Lotus插件] 保存配置失败 [${name}]: ${error.message}`)
      return false
    }
  }
  ...
  watch(filePath, name, type) {
    ...
    try {
      ...
      watcher.on('change', () => {
        ...
        logger.mark(`[Lotus插件] 配置文件已更新: ${name} [${type}]`)
      })
      this.watcher[watchKey] = watcher
    } catch (error) {
      logger.warn(`[Lotus插件] 监听配置文件失败: ${error.message}`)
    }
  }
   ...
   */
  getDataPath(name) {
    const dataDir = path.join(pluginRoot, 'data', 'lotus')
   ...
   */
  setData(name, data) {
    const filePath = this.getDataPath(name)
    try {
      fs.writeFileSync(filePath, JSON.stringify(data, null, 2), 'utf8')
      return true
    } catch (error) {
      logger.error(`[Lotus插件] 保存数据失败 [${name}]: ${error.message}`)
      return false
    }
  }
  ...
  getData(name) {
    ...
    try {
      ...
      logger.error(`[Lotus插件] 读取数据失败 [${name}]: ${error.message}`)
      return null
    }
  }
}
...
// 导出单例
export default new LotusSetting()